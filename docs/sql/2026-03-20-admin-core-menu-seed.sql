...
SELECT
    'base-data', 0, '基础数据', 'MENU', 'BaseData', '/base-data', 'views/base/BaseDataView.vue',
    'Collection', 'page:base-data:view', 0, 0, 4, 1,
    1, 1, NOW(), NOW(), 0, '补齐管理员基础数据菜单'
FROM dual
WHERE NOT EXISTS (
...
WHERE r.role_code = 'ADMIN'
  AND r.deleted = 0
  AND m.menu_code IN ('course-plan', 'schedule', 'base-data', 'guide')
  AND rm.id IS NULL;
